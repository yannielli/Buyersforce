    ...
    def decorator(view):
        ...
        def wrapped(*args, **kwargs):
            ...
            if g.user["role"] != role:
                ...
                return redirect(url_for("home_for_role", role=g.user["role"]))
            return view(*args, **kwargs)
        return wrapped
    return decorator
...
def signup():
    ...
    if request.method == "POST":
        ...
        elif dbm.query("SELECT id FROM users WHERE email = ?", (email,), one=True):
        ...
        if error is None:
            user_id = dbm.execute(
                ...
                "VALUES (?, ?, ?, ?, ?)",
                (role, name, email, generate_password_hash(password), company),
            )
            if role == "seller":
                dbm.execute(
                    ...
                    "'', '', '', '#2a78d6', ?)",
                    (user_id, company, "".join([w[0] for w in company.split()[:2]]).upper() or "VN"),
                )
            session.clear()
            session["user_id"] = user_id
            flash("Welcome to BuyersForce! Your account is ready.", "success")
...
def login():
    ...
    demo_buyers = dbm.query(
        "SELECT * FROM users WHERE role = 'buyer' ORDER BY company, name"
    )
    demo_sellers = dbm.query(
        "SELECT * FROM users WHERE role = 'seller' ORDER BY company, name"
    )
    return render_template("login.html", demo_buyers=demo_buyers, demo_sellers=demo_sellers)


@app.route("/demo-login/<int:user_id>")
def demo_login(user_id):
    user = dbm.query("SELECT * FROM users WHERE id = ?", (user_id,), one=True)
    if user is None:
        abort(404)
    session.clear()
    session["user_id"] = user["id"]
