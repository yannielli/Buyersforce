"""Seed BuyersForce with realistic demo data (PostgreSQL)."""
import os
import secrets
import psycopg2
...
from db import _database_url, _pg
...
def _already_seeded(con):
    """True if the `users` table exists and has at least one row."""
    try:
...
def run(force=False):
    ...
    try:
        if _already_seeded(con) and not force:
            # Real data already lives in this database -- don't clobber it.
        ...
        def insert_id(sql, params=()):
            cur.execute(_pg(sql.rstrip(";")) + " RETURNING id", params)
        ...
        for name, email, category, accent, initials, tagline, desc, site, tags, listings in seller_defs:
            ...
            vendor_ids[name] = vid
            for tag in tags:
                run_sql("INSERT INTO vendor_tags (vendor_id, tag) VALUES (?, ?)", (vid, tag))
        ...
        aegis_seller = fetch_one("SELECT id FROM users WHERE email='sam@aegisshield.io'")[0]
        convo = [
        ...
        aegis_seller_id = fetch_one("SELECT id FROM users WHERE email='sam@aegisshield.io'")[0]
        run_sql(
